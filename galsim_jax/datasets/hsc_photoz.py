...
def stack_bands(cutout, target_size=64):
    ...
    assert (
        cropped.shape[0] == target_size and cropped.shape[1] == target_size
    ), f"Wrong size! Still {cropped.shape}"
    return cropped
...
class HscPhotoz(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        catalog_path = dl_manager.download(
            "https://storage.googleapis.com/ahw2019/hsc_photoz/data/catalog.fits"
        )
        cutouts_path = dl_manager.download(
            "https://storage.googleapis.com/ahw2019/hsc_photoz/data/cutouts.hdf"
        )
        return [
            tfds.core.SplitGenerator(
                name=tfds.Split.TRAIN,
                # These kwargs will be passed to _generate_examples
                gen_kwargs={"catalog_path": catalog_path, "cutouts_path": cutouts_path},
            ),
        ]
